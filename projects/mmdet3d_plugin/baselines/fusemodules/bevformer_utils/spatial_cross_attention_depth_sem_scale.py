...
class SAMDEPTH_AWARE_MSDeformableAttention(BaseModule):
    ...
    def forward(self,
        ...
        if torch.cuda.is_available() and value.is_cuda:
            ...
            print('in sem depth attention input--> pred_img_depth: {}, spatial_shapes: {}, level_start_index: {}, depth_reference_points: {}'.format(
                pred_img_depth.size(), spatial_shapes, level_start_index, depth_reference_points.size()
            ), depth_attention_weights.size(), self.im2col_step)
            ...
            depth_weights = depth_weights.unsqueeze(2).repeat(1, 1, num_points, 1).reshape(bs, num_query,
                                                                                           num_all_points)
            print('depth weights is: ', depth_weights.size(), 'attention_weights is: ', attention_weights.size())
            attention_weights = attention_weights * depth_weights[:, :, None, None, :]

            print('before output is: ', value.size(), 'spatial_shapes is: ', spatial_shapes,
                  'level_start_index is: ', level_start_index, 'sampling_locations is: ', sampling_locations.size(),
