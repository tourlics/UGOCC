...
class NuScenesDatasetOccpancyMultiFrame(NuScenesDataset):
    def __init__(self,
                 ann_file,
                 pipeline=None,
                 data_root=None,
                 ...
                 multi_frame_id_cfg=None,
                 use_render_self_supervised=False
                 ):
        # 调用父类的构造函数
        super().__init__(
            ann_file=ann_file,
        ...
        self.multi_frame_id_cfg = multi_frame_id_cfg
        self.use_render_self_supervised = use_render_self_supervised
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                if self.use_render_self_supervised:
                    info_adj_list = self.get_adjacent_frame_info(info, index)
                    input_dict.update(dict(adjacent=info_adj_list))

        input_dict['occ_gt_path'] = self.data_infos[index]['occ_path']

        # print('input dic tkeys is: ', input_dict.keys())
        return input_dict

    def get_adjacent_frame_info(self, info, index):
        info_adj_list = []
        adj_list = self.multi_frame_id_cfg[1:]
        # print('adadaadadadadad: ', adj_list)
        for select_id in adj_list:
            if index + select_id < 0:
                select_id = 0
            elif index + select_id > len(self.data_infos)-1:
                select_id = len(self.data_infos) - 1
            else:
                select_id = index + select_id
            # print('index and select id isi si si si sis si: ', index, select_id)
        ...
        return info_adj_list
            ...
            # select_id = min(len(self.data_infos)-1, index+select_id)
