...
grid_config = {
    ...
    'z': [-1, 5.4, 6.4],
    # 'z': [-1, 5.4, 0.4],
    'depth': [1.0, 15.0, 0.5],   # 这是一个值得调节的参数
}
...
model = dict(
    type='OnlySSSdDepthSamBaseline',
    num_cams=cam_num_use,
    load_multi_frame=load_multi_frame,
    to_bev=True,
    backproj=dict(
        type='BackprojectDepth2Cam',
        batch_size=batch_size*data_config['Ncams'],
        height=data_config['input_size'][0],
        width=data_config['input_size'][1]
    ),
    projview=dict(
        type='Project2adjView',
        height=data_config['input_size'][0],
        width=data_config['input_size'][1],
        eps=1e-7
    )
...
dataset_type = 'WaymoOccMultiFrame_Real'
...
train_pipeline = [
    ...
    dict(
        type='WaymoNewPrepareImageInputs_AdjFrame',
        is_train=True,
        data_config=data_config,
        sequential=False,
        bda_aug_conf=bda_aug_conf
    ...
    # dict(type='WaymoGenerateViewSegmentationLabel', downsample=1, grid_config=grid_config),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_depth', 'voxel_semantics',
                                'mask_lidar', 'mask_camera', 'adj_img_inputs'],
...
data = dict(
    ...
    train=dict(
        data_root=data_root,
        ann_file=data_root + 'waymo_infos_train_refined.pkl',
        ...
        revised_label=True,
        occ_class_nums=True,
        load_multi_frame=load_multi_frame  # max is 2 because the index -3 will lost most of information
    ),
...
optimizer = dict(type='AdamW', lr=1.1e-4, weight_decay=1e-2)
