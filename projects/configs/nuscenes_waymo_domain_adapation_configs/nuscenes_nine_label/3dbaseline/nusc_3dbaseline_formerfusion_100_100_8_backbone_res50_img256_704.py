...
out_ids = [0, 1, 2]
model = dict(
    type='OCC3dTransformerFusionBaseline',     # single-frame
    to_bev=False,
    Backbone=dict(
        type='ResNet',
        depth=50,
    ...
    FusionMoudle=dict(
        ...
        downsample=16),
    AtentionModule=dict(
        type='Atention_Fusion_Module3D',
        volume_h=int((grid_config['y'][1] - grid_config['y'][0]) / grid_config['y'][2]),
        volume_w=int((grid_config['x'][1] - grid_config['x'][0]) / grid_config['x'][2]),
        volume_z=int((grid_config['z'][1] - grid_config['z'][0]) / grid_config['z'][2]),
        transformer=dict(
            type='OCCFormer',
            embed_dims=_dim_,
            encoder=dict(
...
# load_from = './domain_adaptation_work_dirs/nuscenes_nine_label/3dbaseline/nusc_3dbaseline_depthfusion_2_42_0.5_backbone_res50_img256_704_with_transformer/epoch_8_ema.pth'
