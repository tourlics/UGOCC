...
model = dict(
    type='OCC2dDepth_Former_Baseline',
    to_bev=True,
    Backbone=dict(
        type='ResNet',
        depth=50,
...
# Optimizer
optimizer = dict(type='AdamW', lr=9e-5, weight_decay=1e-2)
...
# load_from = "work_dirs/nusc_2d_head_baseline_img256_704_200_200_16_res50_depth2_42_0.5_with_transformer/epoch_70_ema.pth"
...
# todo: 2024.06.08   先前没有考虑到类别不同所带来的问题，我们是否应该针对标签进行修改， 同时，对于nuscenes本身的白天->nightmare
# todo:  ClearWeather→RainyWeather 和  Singapore→Boston  是不是应该单独做实验？
# todo: 如果是为了说明视角缺失问题，可不可以参考 PCT: PERSPECTIVE CUE TRAINING FRAMEWORK FOR MULTI-CAMERA BEV SEGMENTATION在单独数据集中进行去除
# todo： 最后展示跨数据集的用于炫技？   那这样子是不是应该加入surroundocc进行对比试验
# DeepLabV3+
# Segformer
# Mask2Former   语义伪标签生成方法
