...
from . import Self_Block

@MODELS.register_module()
class CVT(nn.Module):
    def __init__(self, iter_num=4, dim=64, num_heads=4, downsample_ratio=1, self_block=None, positionembedding=None,
                 sep_conv=None, sep_deconv=None):
        super().__init__()
        self.iter_num = iter_num
        self.downsample_ratio = downsample_ratio
        # 动态创建 self_block
        for i in range(iter_num):
            block = MODELS.build(self_block)
            setattr(self, f'self_block_{i}', block)

        self.postional_embed = MODELS.build(positionembedding)
        self.sep_conv = MODELS.build(sep_conv)
        self.sep_deconv = MODELS.build(sep_deconv)

    def forward(self, x):  # B, N, C, H, W
        B, N, C, H, W = x.shape
        # print('1st x is: ', x.size())
        x = self.sep_conv(x.view(-1, C, H, W)).view(B, N, C, H // self.downsample_ratio, W // self.downsample_ratio)
        # print('2st x is: ', x.size())
        for i in range(self.iter_num):
            ...
            # 使用动态创建的 self_block
            x = getattr(self, f'self_block_{i}')(x, x_pos).reshape(
                B, N, C, H // self.downsample_ratio, W // self.downsample_ratio
            )
        # print('3st x is: ', x.size())
        x = self.sep_deconv(x.view(-1, C, H // self.downsample_ratio, W // self.downsample_ratio))
        # print('in transformer x is:: ', x.size())
        x = x.view(B, N, C, H, W)
