...
class BEVDepthOCC(BEVDepth):
    ...
    def forward_train(self,
        ...
        # print('type of the depth_pred is: {}, the gt_depth is: {}'.format(depth.size(), gt_depth.size())
        #       ,gt_depth.max(), gt_depth.min(), torch.any(torch.isnan(gt_depth)).item())
        # print('img_feats[0] is: {}, voxel_semantics is: {}, mask_camera is: {}'.format(
        #     img_feats[0].size(), voxel_semantics.size(), mask_camera.size()))
    ...
    def simple_test(self,
        ...
        occ_list = self.simple_test_occ(occ_bev_feature, img_metas)    # List[(Dx, Dy, Dz), (Dx, Dy, Dz), ...]
        # print('occ list is: ', len(occ_list), occ_list[0].shape)
        return occ_list
