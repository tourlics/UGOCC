...
class OCC_LSSViewTransformerFunction(BaseModule):
    ...
    def __init__(
        ...
        super(OCC_LSSViewTransformerFunction, self).__init__()
...
@MODELS.register_module()
class OCC_LSSViewTransformerFunction3D(BaseModule):
    r"""Lift-Splat-Shoot view transformer.
    ...
    def __init__(
        ...
        super(OCC_LSSViewTransformerFunction3D, self).__init__()
        self.uniform = uniform
        self.with_cp = with_cp
    ...
    def create_frustum(self, depth_cfg, input_size, downsample):
        ...
        # D x H x W x 3
        self.frustum = torch.stack((x, y, d), -1)
        print('self.frustum: ', self.frustum.size())
    ...
    def get_lidar_coor(self, rots, trans, cam2imgs, post_rots, post_trans,
        ...
        B, N, _ = trans.shape
        # print('size of cam2imgs and rots and trans: ', cam2imgs.size(), rots.size(), trans.size())
        # post-transformation
        # B x N x D x H x W x 3
        # print('self.frustum: ', self.frustum.size())
        points = self.frustum.to(rots) - post_trans.view(B, N, 1, 1, 1, 3)
        # print('points1: ', points.size())

        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3) \
            .matmul(points.unsqueeze(-1))
        # print('points2: ', points.size())
        # cam_to_ego
        points = torch.cat(
            (points[..., :2, :] * points[..., 2:3, :], points[..., 2:3, :]), 5)
        # print('points3: ', points.size())
        combine = rots.matmul(torch.inverse(cam2imgs))
        # print('points4: ', points.size())
        points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        points += trans.view(B, N, 1, 1, 1, 3)
        points = bda.view(B, 1, 1, 1, 1, 3,
                          3).matmul(points.unsqueeze(-1)).squeeze(-1)
        # print('points5: ', points.size())
        return points
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths)
        bev_feat = bev_feat.permute(0, 1, 4, 3, 2)  # (B, Dz, Dy, Dx, C)- > B, C, X, Y, Z
        # bev_feat = torch.cat(bev_feat.unbind(dim=2), 1)
        return bev_feat
    ...
    def view_transform_core(self, cam_params, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            # print('coor is++++++++: ', coor.size(), depth.size(), tran_feat.size())

            bev_feat = self.voxel_pooling_v2(
                coor, depth,
                tran_feat)
            # print('134141341',bev_feat.size())
        return bev_feat
    ...
    def forward(self, cam_params, context, depth, **kwargs):
        ...
        Args:
            input (list(torch.tensor)): of (image-view feature, rots, trans,
                intrins, post_rots, post_trans)
            cam_params:   imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda
            context:    the image feature  Tensor    B, N, C, H, W
            depth:  the softmax dealed multi-view depth map
