...
class semformer_encoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, grid_config=None, data_config=None, return_intermediate=False, dataset_type='nuscenes', fix_bug=False,
        ...
        super(semformer_encoder, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
        self.fix_bug = fix_bug
        self.x_bound = grid_config['x']
        self.y_bound = grid_config['y']
        self.z_bound = grid_config['z']
        self.final_dim = data_config['input_size']  # 这里省去了可调节的downsample
    ...
    def point_sampling(self, reference_points, cam_params=None):
        ...
        # print('in the point sampling ++++++++++++', reference_points.size(),
        #       rots.size(), trans.size(), intrins.size(), post_rots.size(), post_trans.size(), bda.size())
        B, N, _ = trans.shape
        eps = 1e-5
        ...
        reference_points_cam[..., 0] /= ogfW   # 这里是为了归一化
        reference_points_cam[..., 1] /= ogfH

        mask = (reference_points_cam[..., 2:3] > eps)  # 深度不为0， 且大于0
        mask = (mask & (reference_points_cam[..., 0:1] > eps) 
                 & (reference_points_cam[..., 0:1] < (1.0-eps)) # 像素坐标在0-1之间，对每一个视角都一样
                 & (reference_points_cam[..., 1:2] > eps) 
                 & (reference_points_cam[..., 1:2] < (1.0-eps)))
        B, N, H, W, D, _ = reference_points_cam.shape
        # print('ref_points camis: ', reference_points_cam.size())
        ...
        # print('in the pointsample final: ', reference_points.size(), reference_points_cam.size(), mask.size())
        return reference_points, reference_points_cam[..., :2], mask, reference_points_cam[..., 2:3]

    def point_sampling_v2(self, reference_points, cam_params=None):
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            output = layer(
                ...
                **kwargs)
            # print('lid is: {}, and size is: {}'.format(lid, output.size()))
            bev_query = output
...
class SEMFormerEncoderLayer(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
                 ...
                 ffn_num_fcs=2,
                 **kwargs):
        super(SEMFormerEncoderLayer, self).__init__(
            attn_cfgs=attn_cfgs,
    ...
    def forward(self,
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        print('self.operation_order is: ', type(self.operation_order), self.operation_order)
        for layer in self.operation_order[:3]:
            # temporal self attention
            if layer == 'self_attn':
                print('-self_attn is: ', self.attentions[attn_index])
                ...
                attn_index += 1
                identity = query
                print('self attention query is: ', query.size())
            elif layer == 'norm':
                print('norm_attn is: ', self.norms[norm_index])
                query = self.norms[norm_index](query)
                norm_index += 1
                print('norm query is: ', query.size())
            # spaital cross attention
            elif layer == 'cross_attn':
                print('cross-attention is: ', self.attentions[attn_index])
                query = self.attentions[attn_index](
                    query,
                    key,
                    value,
                ...
                attn_index += 1
                identity = query
                print('cross query is: ', query.size())
            elif layer == 'ffn':
                query = self.ffns[ffn_index](
                    query, identity if self.pre_norm else None)
                ffn_index += 1
            # print('inner layer the query size is: {}, and layer name is: {}'.format(query.size(), layer))
        return query