...
class Metric_mIoU():
    ...
    def hist_info(self, n_cl, pred, gt):
        ...
        # print('pred {}, gt {}'.format(pred.shape, gt.shape),  np.unique(gt, return_counts=True))
        k = (gt >= 0) & (gt < n_cl) # exclude 255
        # print('after k is: ',  np.unique(gt[k], return_counts=True))
        labeled = np.sum(k)     # N_total   , here we get the non free nums
    ...
    def add_batch(self, semantics_pred, semantics_gt, mask_lidar, mask_camera):
        ...
        """
        # print('semantics_pred {}, semantics_gt {}, mask_lidar {}, mask_camera {} size@@@@@@'.format(
        #     semantics_pred.shape, semantics_gt.shape, mask_lidar.shape, mask_camera.shape
        # ))
        # print('semantics_pred {}, semantics_gt {}, mask_lidar {}, mask_camera {}'.format(semantics_pred.shape, semantics_gt.shape, mask_lidar.shape, mask_camera.shape))
        # print('+++++++++++++++++++++++++++++++++++++++++++in the metric occ: ', type(semantics_pred), type(semantics_gt))
        self.cnt += 1
        ...
        # print('hist is: ', _hist)
        # print('hist size is: {}'.format(type(_hist)), _hist.shape)
