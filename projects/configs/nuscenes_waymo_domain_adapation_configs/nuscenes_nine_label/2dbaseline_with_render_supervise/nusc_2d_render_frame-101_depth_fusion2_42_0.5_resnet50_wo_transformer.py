...
multi_frame_id_cfg = [0, -1, 1]
use_render_self_supervised = True
...
model = dict(
    type='DGA_OCC_PIPLINE',
    to_bev=True,
    grid_config=grid_config,
    num_frame=len(multi_frame_id_cfg),
    Backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
dataset_type = 'NuScenesDatasetOccpancyMultiFrame'
...
train_pipeline = [
    ...
    dict(
        type='Collect3D', keys=['points', 'img_inputs', 'gt_depth', 'voxel_semantics',
                                'mask_lidar', 'mask_camera', 'lidarego2global'])
]
...
share_data_config = dict(
    ...
    multi_frame_id_cfg=multi_frame_id_cfg,
    use_render_self_supervised=use_render_self_supervised
)
