...
class Atention_Fusion_Module(BaseModule):
    """Head of Detr3D.
    Args:
    ...
    def forward(self, mlvl_feats, img_metas, lss_bev=None, gt_bboxes_3d=None, cam_params=None, pred_img_depth=None,
                bev_mask=None):
        """Forward function.
        Args:
        ...
        # print('+++++++this is the inputs of bp: +++++++++++', mlvl_feats[0].size(),
        #       type(img_metas), lss_bev.size(), type(cam_params), pred_img_depth.size(), type(bev_mask))
        bs, num_cam, _, _, _ = mlvl_feats[0].shape
        dtype = mlvl_feats[0].dtype
        bev_queries = self.bev_embedding.weight.to(dtype)
        # print('orginal bev_queries size is: {}'.format(bev_queries.size()))
        bev_queries = bev_queries.unsqueeze(1).repeat(1, bs, 1)
        # print('orginal lssbev size is: {}'.format(lss_bev.size()))
        # print('img size is: {}'.format(mlvl_feats[0].size()))
        # it is true this network can work well without lss_forefeature
        ...
        # print('orginal bevpos size is: {}'.format(bev_pos.size()))
        ...
        # print('in the attention fusion is: bev--- {}'.format(bev.size()))
        return bev
