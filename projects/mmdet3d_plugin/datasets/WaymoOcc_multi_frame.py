...
class WaymoOccMultiFrame_Real(Dataset):
    def __init__(self,
                 data_root,
                 ann_file,
                 load_interval=5,
    ...
    def get_data_info(self, index):
        ...
        if self.img_info_prototype == 'mmcv':
            ...
            if self.load_multi_frame != 0:
                info_adj_list = self.get_adj_info(info, index)
                input_dict.update(dict(adjacent_frame=info_adj_list))

        # print('input_dict keys is: ', input_dict.keys(), input_dict['curr'].keys(),
        #       input_dict['adjacent_frame'].keys())
        # print('pre and next: ', [input_dict['adjacent_frame'][str(i)].keys() for i in
        #                          list(range(-self.load_multi_frame, 0)) + list(range(1, self.load_multi_frame+1))])

        # print('debug dataset length: ', len(self.data_infos), len(self.data_infos_full))
        return input_dict


    def get_adj_info(self, info, index):
        adj_frame = self.load_multi_frame
        pre_load_index = list(range(-adj_frame, 0))
        next_load_index = list(range(1, adj_frame+1))
        adj_dict = dict()

        # print('self.load_interval index', self.load_interval, index)
        for idx in pre_load_index:
            if self.load_interval*index + idx < 0 or self.load_interval*index + idx >= len(self.data_infos_full):
                info_pre = self.data_infos_full[self.load_interval * index]
            else:
                info_pre = self.data_infos_full[self.load_interval*index + idx]
            if info_pre['curr_scene_idx'] == info['curr_scene_idx']:
                adj_dict[str(idx)] = info_pre
            else:
                adj_dict[str(idx)] = info

        for idx in next_load_index:
            if self.load_interval*index + idx < 0 or self.load_interval*index + idx >= len(self.data_infos_full):
                info_next = self.data_infos_full[self.load_interval * index]
            else:
                info_next = self.data_infos_full[self.load_interval*index + idx]
            # info_next = self.data_infos_full[self.load_interval*index + idx]
            if info_next['curr_scene_idx'] == info['curr_scene_idx']:
                adj_dict[str(idx)] = info_next
            else:
                adj_dict[str(idx)] = info
